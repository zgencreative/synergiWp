...
(function($) {
  ...
  $(function($) {
    ...
    var group = '.form-group.mfn-pills-field';

    function init(){

      $(group).not('.pill-initialized').each(function(){
    ...
    function PillField(domLocation){
      ...
      this.refreshHiddenInput = function() {
        ...
        that.hiddenInputDOM.attr('value', that.pills.join(' '));
      ...
      this.pushWordToArray = function() {
        ...
        if( that.hiddenInputDOM.attr('name').indexOf('tags_anchors') !== -1 ){
    ...
    $(document).on('mfn:vb:sc:edit', function() {
      $('.modal-add-shortcode .pill-initialized').removeClass('pill-initialized');
