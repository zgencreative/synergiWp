...
if( 'footer' == mfn_opts_get('css-location') ){
	add_action('get_footer', 'mfn_styles', 11);
} else {
...
if( 'footer' == mfn_opts_get('css-location') ){
	add_action('get_footer', 'mfn_styles_static', 11);
} else {
...
if( 'footer' == mfn_opts_get('css-location') ){
	add_action('get_footer', 'mfn_styles_inline', 100);
} else {
...
if( 'footer' == mfn_opts_get('css-location') ){
	add_action('get_footer', 'mfn_styles_custom', 101);
} else {
...
function mfn_visual_builder(){
	...
	if( ! empty( $_GET['visual'] ) && $_GET['visual'] == 'iframe' ){
		...
		function mfnvb_body_classes( $classes ) {
		    ...
		    if( get_post_type( get_the_ID() == 'template' ) &&  get_post_meta(get_the_ID(), 'mfn_template_type', true) == 'header' ){
		    	$classes[] = 'mfn-header-tmpl';
		    }
	...
	function mfnvb_admin_bar_menu( $admin_bar ){
		if( is_singular(['page','post','portfolio','product']) ){
		    ...
		    'title' => 'Edit with '. apply_filters('betheme_label', 'Muffin') .' Live Builder', // Label for this item
		    'href' => admin_url('/post.php?post='. get_the_ID() .'&action='. apply_filters('betheme_slug', 'mfn') .'-live-builder'),
			);
