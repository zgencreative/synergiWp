	...
	if( mfn_opts_get( 'logo-img' ) || $logo_text || ( ! has_custom_logo() ) ){
		...
		if ( $logo_text ) {
			...
			foreach ( $logo['default'] as $logo_key => $logo_src ) {
				echo '<img class="logo-'. esc_attr( $logo_key ) .' scale-with-grid '. esc_attr( $svg ) .'" src="'. esc_url( $logo_src ) .'" data-retina="'. esc_url( $logo['retina'][$logo_key] ) .'" data-height="'. esc_attr( mfn_get_attachment_data( $logo_src, 'height' ) ) .'" alt="'. esc_attr( mfn_get_attachment_data( $logo_src, 'alt' ) ) .'" '. $width .' data-no-retina/>';
			}
